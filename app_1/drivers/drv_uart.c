...
static struct uart_device uart1 =
{
    ...
    .rx_len = 0,
    .index_num = 0,
    .gpio_rx = {.port = IO_PORTB, .pin = PIN_9, .mux = IO_PERIPH_MUX7, .pull = IO_PULL_RESERVE},
    .gpio_tx = {.port = IO_PORTB, .pin = PIN_8, .mux = IO_PERIPH_MUX7, .pull = IO_PULL_RESERVE},
    .paret.ops = &_uart_ops,
};
...
int rt_hw_uart_init(void)
{
    ...
    rt_memset(&uart1.rx_buf[0], 0, sizeof(uart1.rx_buf));
    rt_hw_interrupt_install(uart1.irq, uart_irq_handler, &uart1, "uart1");
    rt_hw_interrupt_umask(uart1.irq);

    uart1.paret.config = default_config;
    ret = rt_hw_serial_register(&uart1.paret, "uart1", RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX, &uart1);
#endif /* BSP_USING_UART1 */
