...
unsigned xformat(void (*outchar)(char), const char *fmt, ...)
{
    ...
    va_start(list, fmt);
    count = xvformat(outchar, fmt, list);
    va_end(list);
 ...
 * @return The number of char emitted.
 */
unsigned xvformat(void (*outchar)(char), const char *fmt, va_list _args)
{
    ...
    while (*fmt) {
        ...
        switch (param.state) {
            ...
            case ST_TYPE:
                ...
                param.count += outBuffer(outchar, param.prefix, param.prefixlen, param.flags & FLAG_UPPER);
                if (!(param.flags & FLAG_LEFT))
                    param.count += outChars(outchar, param.pad, param.width);
                param.count += outBuffer(outchar, param.out, param.length, param.flags & FLAG_UPPER);
                if (param.flags & FLAG_LEFT)
                    param.count += outChars(outchar, param.pad, param.width);
        }
    }
