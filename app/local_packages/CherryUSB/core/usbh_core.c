...
int usbh_enumerate(struct usbh_hubport *hport)
{
    ...
    uint8_t string_buffer[sizeof(struct usb_string_descriptor)];

    /* Get Manufacturer string */
    memset(string_buffer, 0, sizeof(struct usb_string_descriptor));
    ret = usbh_get_string_desc(hport,  hport->device_desc.iManufacturer, string_buffer);
    if (ret < 0) {
    ...
    /* Get Product string */
    memset(string_buffer, 0, sizeof(struct usb_string_descriptor));
    ret = usbh_get_string_desc(hport, hport->device_desc.iProduct, string_buffer);
    if (ret < 0) {
    ...
    memset(string_buffer, 0, sizeof(struct usb_string_descriptor));
    ret = usbh_get_string_desc(hport, hport->device_desc.iSerialNumber, string_buffer);
    if (ret < 0) {
