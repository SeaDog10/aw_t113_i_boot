...
static inline void usb_ehci_qh_qtd_flush(struct ehci_qh_hw *qh)
{
    ...
    while (qtd) {
        usb_dcache_clean((uintptr_t)&qtd->hw, USB_ALIGN_UP(SIZEOF_EHCI_QH, CONFIG_USB_EHCI_ALIGN_SIZE));

        if(qtd->bufaddr) {
            if (!qtd->dir_in) {
                usb_dcache_flush(qtd->bufaddr, USB_ALIGN_UP(qtd->length, CONFIG_USB_ALIGN_SIZE));
            }
            else {
                usb_dcache_invalidate(qtd->bufaddr, USB_ALIGN_UP(qtd->length, CONFIG_USB_ALIGN_SIZE));
            }
        }
...
int usb_hc_init(struct usbh_bus *bus)
{
    ...
    for (uint8_t index = 0; index < CONFIG_USB_EHCI_QH_NUM; index++) {
        ...
        if ((uint32_t)(uintptr_t)&qh->hw % 32) {
    ...
    for (uint8_t index = 0; index < CONFIG_USB_EHCI_QTD_NUM; index++) {
        ...
        if ((uint32_t)(uintptr_t)&qtd->hw % 32) {
